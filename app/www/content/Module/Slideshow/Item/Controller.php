...
class Controller extends Stackbox\Module\ControllerAbstract
{
    ...
    {
        $kernel = \Kernel();
        $items = $kernel->mapper()->all('Module\Slideshow\Item\Entity')
    ...
    public function editAction($request, $page, $module)
    {
        ...
        if(!$module) {
            $module = $mapper->get('Module\Slideshow\Item\Entity');
            $form->method('POST');
        }
    ...
    {
        $mapper = $this->kernel->mapper();
        $item = $mapper->get('Module\Slideshow\Item\Entity')->data($request->post());
    ...
    {
        $mapper = $this->kernel->mapper();
        $item = $mapper->get('Module\Slideshow\Item\Entity', (int) $request->item);
        if(!$item) {
            return false;
        }
    ...
    {
        $mapper = $this->kernel->mapper();
        $item = $mapper->get('Module\Slideshow\Item\Entity', $request->module_item);
        if(!$item) {
            return false;
        }
    ...
    {
        $view = $this->kernel->spotForm($this->kernel->mapper()->get('Module\Slideshow\Item\Entity'));
        $fields = $view->fields();
        
        // Set text 'content' as type 'editor' to get WYSIWYG
        $fields['url']['after'] = '<a href="#">Select Image...</a>';
